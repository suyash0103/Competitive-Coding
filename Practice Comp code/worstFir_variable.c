...
int worst(struct Node **head, int req_size)
{
    int flag = 0;
    ...
    struct Node* temp_back = NULL;
    temp_back = (struct Node*)malloc(sizeof(struct Node));
    ...
    while(pointer2 != NULL)
    {
        ...
        else if(avail_size == req_size && avail_size > (temp -> end - temp -> start))
        {
            temp = pointer2;
            temp_back = pointer1;
            flag = 1;
    ...
    if(flag == 1)
    {
        if(temp -> end - temp -> start > req_size)
        {
            temp -> start += req_size;
        }
        else if(temp -> end - temp -> start == req_size)
        {
            if(temp == (*head))
            {
                (*head) = (*head) -> next;
